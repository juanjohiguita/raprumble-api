...
export const updateUser = async (req, res) => {
    ...
    try {
        const success = await userServices.updateUserById(id, userData);
...
export const getUserAllInformation = async (req, res) => {
    const id = req.params.id;
    try {
        const user = await userServices.getUserAllInformationById(id);
        res.json(user);
    } catch (error) {
        res.status(500).json({ message: error.message || "Error in the server" });
    }
...
export const updateUserAllInformation = async (req, res) => {
    ...
    const userData = req.body;
    try {
        const updatedUser = await userServices.updateUserAllInformationById(id, userData);
...
export const updateUserAka = async (req, res) => {
    ...
    const { aka } = req.body;
    try {
        const updatedUser = await userServices.updateUserAkaById(id, { aka });
